...
export default function DashboardMedicoScreen() {
  ...
  useEffect(() => {
    const buscarConsultas = async () => {
      ...
      if (userSalvo && token) {
        ...
        setNomeMedico(user.nome);

        if (user.medicoId) {
          const consultas = await consultaService.obterPorMedicoId(user.medicoId, token);
  ...
  const consultasDoDia = consultas.filter(c =>
    dayjs(c.dataHora).isSame(dataSelecionada, 'day')
  );

  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.nome}>Olá, Dr(a). {nomeMedico}</Text>
              ...
              return (
                <View key={index} style={styles.diaContainer}>
                  ...
                  <TouchableOpacity
                    ...
                    style={[styles.numeroWrapper, isToday && styles.hojeCircle]}
                  >
                    <Text
                      style={[styles.numero, isToday && styles.numeroHoje]}
                    >
                      {dia.format('D')}
                    </Text>
                  </TouchableOpacity>
      ...
      <ScrollView style={styles.consultasContainer}>
        {consultasDoDia.map((c, i) => (
          <View key={i} style={styles.consulta}>
            <View style={styles.consultaCabecalho}>
              <Text style={styles.nomePaciente}>Paciente: {c.paciente?.nome || '---'}</Text>
...
const styles = StyleSheet.create({
  ...
  nome: { fontSize: 18, fontWeight: 'bold', marginBottom: 8, color: '#fff' },
  consultaCabecalho: {
    flexDirection: 'row',
